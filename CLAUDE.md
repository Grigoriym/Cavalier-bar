...
- `GirCore.Adw-1` is **intentionally left pinned at `0.5.0-preview.3`** (latest stable is `0.8.1`). Version `0.8.0` reworked `Gtk.Builder` into a composite-template/source-generator pattern (`[GObject.Subclass<T>]` + `[Gtk.Template<...>]`), breaking the `[Gtk.Connect]` + `builder.GetPointer(name)`/`builder.Connect(this)` usage this app relies on in every window/dialog (`MainWindow.cs`, `DrawingView.cs`, `PreferencesDialog.cs`, `CommandHelpDialog.cs`) plus the raw `Handle` (now a typed `ObjectHandle`, not `nint`) interop in `Helpers/GtkHelpers.cs`. Bumping it compiles with ~18 errors across those 5 files — a real view-layer migration to the new composite-template pattern, not a version bump. Left for a dedicated pass if ever done.
